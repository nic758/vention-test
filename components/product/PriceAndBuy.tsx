...
import {IProduct, IProductVariant} from 'boundless-api-client';
import clsx from 'clsx';
import {useAppDispatch} from '../../hooks/redux';
import {updateItem2Cart} from '../../redux/actions/cart';
...
import {showCall2Order} from '../../redux/reducers/cart';
...
export default function ProductPriceAndBuy({product, selectedVariant, setError, onAddedToCart}: IPriceAndBuyProps) {
	...
	const onBuyBtnClicked = (e: MouseEvent<HTMLButtonElement>) => {
		...
		dispatch(updateItem2Cart(product, qty));

		if (onAddedToCart) {
			onAddedToCart(itemId, qty);
		}
		dispatch(showCall2Order({product}));
	};
